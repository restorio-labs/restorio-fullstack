...
Goals
RPO ≤ 1 hour — maximum acceptable data loss window via continuous WAL/oplog archiving
RTO ≤ 4 hours — full restore from offsite to operational state
3-2-1 compliance — 3 copies, 2 media types, 1 offsite (cross-region)
Verified backups — automated weekly restore tests; unverified backups are assumptions
Encryption everywhere — at rest (AES-256 / SSE-S3) and in transit (TLS)
Retention Policy
